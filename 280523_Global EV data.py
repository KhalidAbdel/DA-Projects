...
chargingpoints_count = chargingpoints_count.reset_index()
grouped_data = chargingpoints_count.groupby('region')
for region, region_data in grouped_data:
    pyplot.plot(region_data['year'], region_data['value'], label=region)

pyplot.xlabel('Year')
pyplot.ylabel('Number of charging points')
pyplot.title('Charging points by Region and Year')
pyplot.legend()
pyplot.grid(True)
pyplot.show()
